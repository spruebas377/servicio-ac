...
const UserCardWrapper = styled(Card)(({ theme, featured }) => ({
  borderRadius: "1.25rem",
  border: featured
    ? `2px solid ${theme.palette.warning.main}`
    : `1px solid ${theme.palette.mode === "light" ? alpha("#000", 0.08) : alpha("#fff", 0.08)}`,
  backgroundColor:
...
const ImageOverlay = styled(Box)(({ theme }) => ({
  ...
  inset: 0,
  backgroundColor:
    theme.palette.mode === "light" ? "rgba(0,0,0,0.45)" : "rgba(0,0,0,0.65)",
  display: "flex",
  alignItems: "center",
...
const AgeFilterButton = styled(Button)(({ theme, active }) => ({
  textTransform: "none",
  fontWeight: 500,
  fontSize: "0.85rem",
  borderRadius: "1rem",
  ...
  backgroundColor: theme.palette.background.paper,
  border: `1px solid ${active ? alpha(theme.palette.text.primary, 0.3) : theme.palette.divider}`,
  transition: "all 0.2s ease",
  "&:hover": {
...
const ServiceFilterButton = styled(Button)(({ theme, active }) => ({
  textTransform: "none",
  fontWeight: 500,
  fontSize: "0.85rem",
  borderRadius: "1rem",
...
const sortByAge = (list) =>
  [...list].sort(
    (a, b) => (Number(a.age) || Infinity) - (Number(b.age) || Infinity),
  );
...
const UserResultCard = ({
  ...
  const fetchUserBucketImages = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        console.warn(`Error al listar imágenes ${user.id}:`, error);
        setImages([]);
        return;
      }
      ...
      console.error(`Error en fetchUserBucketImages ${user.id}:`, err);
    } finally {
      setLoadingImages(false);
    }
  }, [user?.id]);
  ...
  return (
    <UserCardWrapper featured={!!user.featured}>
      <CardHeader
        ...
        action={
          <Tooltip title="Actualizar fotos">
            <IconButton
              size="small"
      ...
      <Box sx={{ px: 2, pb: 1 }}>
        <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
          ...
          <Chip
            ...
            label={`${images.length} ${images.length === 1 ? "imagen" : "imágenes"}`}
            size="small"
...
const ServicesPopover = ({
  ...
  /* LOG: ver qué llega al popover */
  console.log("🎛️ ServicesPopover → services:", services, "loading:", loading);
      ...
      <Box sx={{ p: 2.5, minWidth: 320, maxWidth: 380 }}>
        ...
        >
          ...
          >
            {localValue.length === 0
              ? "Todos"
              : `${localValue.length} seleccionado${localValue.length === 1 ? "" : "s"}`}
          </Typography>
        </Stack>
          ...
          {loading ? (
            ...
            <Box sx={{ px: 1, py: 2, textAlign: "center" }}>
              ...
              <Typography sx={{ fontSize: "0.72rem", color: "text.disabled" }}>
                Verificá la tabla <code>services</code> en Supabase
              </Typography>
            </Box>
          ) : (
...
export default function SearchResults() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      console.log("📡 Fetching users for gender:", genderId);
      ...
      if (userIds.length > 0) {
        ...
        console.log("✅ USER_SERVICES DATA:", usData);
        console.log("❌ USER_SERVICES ERROR:", usError);
  ...
  const handleCopyUrl = (url) => {
    ...
    setToast({ open: true, message: "¡URL copiada!", severity: "success" });
  };
  ...
  const filteredCities = useMemo(
    () =>
      cities.filter(
        (c) =>
          !selectedProvince ||
          String(c.provincia_id) === String(selectedProvince),
      ),
  ...
  const filteredUsers = useMemo(() => {
    return users.filter((user) => {
      ...
      if (isAgeFilterActive) {
        ...
        if (!userAge || Number.isNaN(userAge)) matchesAge = false;
        else matchesAge = userAge >= ageRange[0] && userAge <= ageRange[1];
      }
    ...
    <Container maxWidth="lg" sx={{ py: { xs: 4, md: 6 } }}>
      ...
      {genderId && (
        <FiltersBar>
          ...
          {/* DEBUG: mostrar contador de servicios cargados */}
          <Typography
        ...
        <EmptyState
          ...
          description={
            hasAnyFilter
              ...
              : `No encontramos usuarios con el género ${activeGender?.name || genderId}. Probá con otro filtro.`
          }
          action={
            hasAnyFilter ? (
              <Button
...
function EmptyState({ icon, title, description, action }) {
  return (
    ...
    >
      ...
      <Typography
        variant="h6"
        sx={{ fontWeight: 600, letterSpacing: "-0.02em", fontSize: "1.1rem" }}
      >
        {title}
      </Typography>
      <Typography
